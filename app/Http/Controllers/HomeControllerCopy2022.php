...
//dd($pgoptions);
      ...
      
  {
    ...
    if($adscsl==996||$adscsl==997||$adscsl==998){
    ...
    public function getoptionnew()
    {
        ...
        $year=date('Y');
            ...
            WHERE 
              ...
              tb_admnscheme.adsc_pgm_sl = tb_program.pgm_sl AND
              tb_centre.centre_sl = asw_seatmatrix.centre  AND  tb_admnscheme.adsc_sl=?  AND tb_admnscheme.adsc_admnyear=?",[$adscsl,$year]);
...
//          dd($cent_opt2);
        ...
        $centrop2=DB::select("SELECT distinct getcentrename(centre) as centre_name,centre as centre_sl FROM admn22.asw_seatmatrix
                where  centre NOT IN(?,?) and pgm=?",[$cent_opt,$cent_opt2,$pgmsl]);
...
//          dd($cent_opt2);
        ...
        $centrop2=DB::select("SELECT distinct getcentrename(centre) as centre_name,centre as centre_sl FROM admn22.asw_seatmatrix
                where  centre NOT IN(?,?,?) and pgm=?",[$cent_opt,$cent_opt2,$cent_opt3,$pgmsl]);
...
//        dd($centrop2);
        ...
        <select  id= \"center_sl5\" name=\"center_sl5\" class=\"form-control form-control-sm select2 col-sm-8\"  >
...
//          dd($center_sl1);
    ...
    if(count($option)==0)
    {
        if($center_sl2 == "" &&  $center_sl3== "" &&  $center_sl4== "" &&  $center_sl5== "")
        {
            DB::insert('insert into  tb_pg_pgpgm (pgpgm_pgapp_id,pgpgm_adsc_sl,pgpgm_centre_sl,pg_option)VALUES(?,?,?,?)',
           [$appid,$adscsl,$center_sl1,$opt1]); 
        }
        else if($center_sl3== "" &&  $center_sl4== "" &&  $center_sl5== "")
        {
            DB::insert('insert into  tb_pg_pgpgm (pgpgm_pgapp_id,pgpgm_adsc_sl,pgpgm_centre_sl,pg_option)VALUES(?,?,?,?)',
               [$appid,$adscsl,$center_sl1,$opt1]);
            DB::insert('insert into  tb_pg_pgpgm (pgpgm_pgapp_id,pgpgm_adsc_sl,pgpgm_centre_sl,pg_option)VALUES(?,?,?,?)',
...
//                ->get();
    ...
    public function pay_details()
    {
        ...
        $payment_success_count = PaymentTrans::where('client_code',Auth::user()->pgapp_id)->where('ucity_service',"PG-ENTRANCE-FEE-2023")->where('res_verified',"SUCCESS")->count();
       ...
       $special_reserv_flag=0;
         ...
         $payf=0;
          if (empty($allotstatusres))
          {
              ...
              else if($allot=='THIRD' || $court_judge==1 || $allot=='THIRD_SPECIAL'){
                  $allotstat=3;
              }
              ...
              else if($allot=='Judgement in WP(C) No.21770 of 2022 dated,07.07.2022' || $allot=='Judgement in WP(C) No. 22770 of 2022 dated,13.07.2022'|| $allot=='SC/ST RE-NOTIFICATION-II' || $allot=='SC/ST RE-NOTIFICATION'
                      || $allot=='Judgement dated 27.07.2022 in WP(C) 24415/2022')
              {
                  $allotstat=5;
                  $payf=1;
              }
       ...
       $sub=DB::select("select pgapp_adsc_sl from tb_pgapp where  pgapp_sl=?",[$pgapp_sl]);
        foreach ($sub as $value1) {
           ...
           //Museology
            ...
            if($pgapp_adsc_sl=="998") //Masters Programme in Disaster Management and Mitigation
            ...
            if($pgapp_adsc_sl=="996") //Post Graduate Diploma in Active Ageing and Wellness Rehabilitation
...
//                    'pgsign','curnt_date','time','pgoptions','semester_grades','payment_success'));
            ...
            $pdf = PDF::loadView('applnprintout',compact('pgapp','religion','community','caste','subcaste','pgquali','pgotherinfo','pgphoto',
                    'pgsign','curnt_date','time','pgoptions','semester_grades','payment_success','name','pflag','pmodeoption'));
