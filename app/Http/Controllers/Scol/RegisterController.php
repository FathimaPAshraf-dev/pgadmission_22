...
namespace App\Http\Controllers\Scol;
...
// $result = DB::select("select *  from tb_admnscheme where adsc_sl IN (752,749,756,755,758,750,757,747,744,748,760,751,761,746,745,742,743,815)");
        ...
        FROM 
          ...
          public.tb_program
        WHERE 
          tb_admnscheme.adsc_pgm_sl = tb_program.pgm_sl  and  tb_admnscheme.adsc_admnyear='2024' and adsc_name not like '%DIP%' and  adsc_name not like '%SPORTS%' 
          and adsc_name not like '%DMM%' and adsc_name not like '%MSC%' and adsc_name not like '%MITIGATION%' and adsc_name not like '%MFA%' and adsc_name not like '%SOCIAL WORK%' ");
...
//YEAR 2020//  (752,749,756,755,758,750,757,747,744,748,760,751,761,746,745,742,743,815)             // 2021 = 820,821,823,824,825,826,827,828,829,830,831,832,834,835,836,837,838,840,842,841
    ...
    if($pgstream_name=="2") // MSW
    {
        ...
        FROM 
          ...
          public.tb_program
        WHERE 
          tb_admnscheme.adsc_pgm_sl = tb_program.pgm_sl  and  tb_admnscheme.adsc_admnyear='2024' and  adsc_name  like '%WORK%' and  adsc_name not like '%AND DMM%' ");
           //819,905
    }
    ...
    if($pgstream_name=="3") // M.PES
    {
        ...
        FROM 
          ...
          public.tb_program
        WHERE 
          tb_admnscheme.adsc_pgm_sl = tb_program.pgm_sl  and  tb_admnscheme.adsc_admnyear='2024' and  adsc_name  like '%SPORTS%' ");
     //907
    }
...
//        $result = DB::select("select *  from tb_admnscheme where adsc_sl IN (763)");
        ...
        FROM 
          ...
          public.tb_program
        WHERE 
          tb_admnscheme.adsc_pgm_sl = tb_program.pgm_sl  and  tb_admnscheme.adsc_admnyear='2024' and  adsc_name  like '%MFA%' and  adsc_name  like '%VISUAL%' ");
       //846,906
    }
    ...
    if($pgstream_name=="8") // MSC
    {
      ...
      //  $result = DB::select("select *  from tb_admnscheme where adsc_sl IN(753,754)");
        ...
        FROM 
          ...
          public.tb_program
        WHERE 
          tb_admnscheme.adsc_pgm_sl = tb_program.pgm_sl  and  tb_admnscheme.adsc_admnyear='2024' and adsc_name like '%MSC%' and  adsc_name not like '%DMM%' ");
    ...
    if($pgstream_name=="9")
    {
        $result = DB::connection('pgsql2')->select("SELECT 
          ...
          FROM 
            ...
            public.tb_program
          WHERE 
            tb_admnscheme.adsc_pgm_sl = tb_program.pgm_sl  and  tb_admnscheme.adsc_admnyear='2024' and  adsc_name  like '%DIP%' ");
    ...
    if($pgstream_name=="12")
    {
        $result = DB::connection('pgsql2')->select("SELECT 
          ...
          FROM 
            ...
            public.tb_program
          WHERE 
            tb_admnscheme.adsc_pgm_sl = tb_program.pgm_sl  and  tb_admnscheme.adsc_admnyear='2024' and  adsc_name  like '%DISASTER%'");
    ...
    if($pgstream_name=="13")
    {
        $result = DB::connection('pgsql2')->select("SELECT 
          ...
          FROM 
            ...
            public.tb_program
          WHERE 
            tb_admnscheme.adsc_pgm_sl = tb_program.pgm_sl  and  tb_admnscheme.adsc_admnyear='2024' ");
       ...
       $mobile=DB::SELECT("SELECT  * from tb_pgapp where pgapp_adsc_sl=? and pgapp_adhar=?",[$user->pgapp_adsc_sl,$user->pgapp_adhar]   ) ;     
        ...
        //dd ($mobile); 
         foreach($mobile as $key){
            ...
            $mes="SCOL ADMISSION ".date("Y")."  Application ID is: {$pgapp_id}  and Password : {$pgapp_password} ";
            //
...
//       $admission= DB::connection('pgsql2')->select("select adsc_sl,adsc_name from tb_admnscheme where adsc_sl in(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)",[752,749,756,755,758,750,757,747,753,754,744,763,748,760,751,761,762,746,745,742,743,759,765,815]);
        $dt_now = Carbon::now();
               $scol_courses= DB::connection('pgsql2')->select("select * from scol.courses ");
        $trn_date= $dt_now->toDateString();
        date_default_timezone_set('Asia/Calcutta');
        $datenow = date("d/m/Y");
  
//        if($datenow>'20/04/2023'){
            //  return view('auth.registerclose'); 
//        }
//        else{
//        }
//        
        return view('scol.register',compact('datenow','scol_courses'));
    }
}
